...
public class PlayerEvents implements Listener {
    ...
    ItemStack getPumpkin() {
        ...
        if (!ServerVersion.get.roundedFromServer().isLegacy()) pumpkin = new ItemStack(Material.CARVED_PUMPKIN);
        ItemMeta pMeta = pumpkin.getItemMeta();
        pMeta.setDisplayName("§c§lReally Inconvenient Hat!");
        if (ServerVersion.get.roundedFromServer().getId() > 10) pMeta.addEnchant(Enchantment.BINDING_CURSE, 1, true);
