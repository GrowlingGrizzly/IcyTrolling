...
public class CmdLevitate implements TabExecutor {
                        ...
                        if (StringUtils.isNumeric(args[1])) {
                            ...
                            if (0 < num && num < 5) {
                                if (!t.hasPotionEffect(PotionEffectType.LEVITATION)) {
                                    t.addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION, 99999999, num));
                            ...
                            } else sender.sendMessage("§cLevitation multiplier cannot be bigger than four.");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 2) return new ArrayList<>(Arrays.asList("1", "2", "3", "4", "off"));
